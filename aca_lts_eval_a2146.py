...
def offsets_for_roll(roll):
    roll_table = Table.read(
        ...
        format='ascii',
        names=['week', 'roll_min', 'roll_max', 'roll_mid', 'y_offset', 'z_offset'])
    rows = roll_table[(roll_table['roll_min'] <= roll) & (roll_table['roll_max'] >= roll)]
    return (rows['y_offset'][0], rows['z_offset'][0])
...
def get_t_ccd_roll(ra, dec, cycle, detector, too, pitch, time, cone_stars):
    """
...
def make_target_report(ra, dec, cycle, detector, too, 
